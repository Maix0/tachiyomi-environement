{
  ...
  inputs.tachiyomi-extensions.url = "github:Maix0/tachiyomi-extensions?branch=enryu-v2";
  ...
  outputs = {
    ...
    flake-utils.lib.eachDefaultSystem (
      system: let
        ...
        tachiyomi-apk-renamed = pkgs.stdenv.mkDerivation {
          name = "tachiyomi-apk-renamed";
          src = tachiyomi-apk;
          passthru.packageName = "eu.kanade.tachiyomi";
          passthru.apkLocation = "out.apk";
          dontUnpack = true;

          installPhase = ''
            mkdir -p $out/
            cp ${tachiyomi-apk} "$out/out.apk"
          '';
        };
      in rec {
        lib = {
          inherit defaultSdkArgs buildExtension;
        ...
        apps = {
          enryu-extension = flake-utils.lib.mkApp {
            drv = lib.runEmulator {
              name = "enryu-emulator";
              installed = [tachiyomi-apk-renamed packages.enryu-apk];
              ...
              sdkArgs = defaultSdkArgs // {abiVersions = ["x86_64"];};
            };
          };
        };
